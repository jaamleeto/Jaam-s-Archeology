...
public class ArthropodsIdolRightclickedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (JaamsArcheologyCommonConfiguration.IDOLS.get() == true) {
			...
			if (world.isClientSide()) {
				if (world.isClientSide())
					Minecraft.getInstance().gameRenderer.displayItemActivation(new ItemStack(JaamsArcheologyModItems.ARTHROPODS_IDOL.get()));
			}
			...
			if (JaamsArcheologyCommonConfiguration.ARTHROPODSVIGOR.get() == true && !world.isClientSide()) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(JaamsArcheologyModMobEffects.ARTHROPODS_VIGOR.get(), 800, 0));
			} else {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(MobEffects.JUMP, 800, 1));
			}
			if (!world.isClientSide()) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
