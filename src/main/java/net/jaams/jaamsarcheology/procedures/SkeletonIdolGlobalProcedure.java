...
public class SkeletonIdolGlobalProcedure {
	...
	@SubscribeEvent
	public static void onSkeletonEntityAttacked(LivingAttackEvent event) {
		Entity defender = event.getEntity();
		Entity attacker = event.getSource().getEntity();
		if (defender instanceof LivingEntity livingDefender && attacker instanceof LivingEntity livingAttacker && livingDefender.hasEffect(JaamsArcheologyModMobEffects.SKELETAL_VIGOR.get())) {
			Vec3 position = defender.position();
			LevelAccessor world = defender.level();
			List<Mob> skeletons = world.getEntitiesOfClass(Mob.class, new AABB(position, position).inflate(15), e -> (e instanceof AbstractSkeleton) && e != defender);
			for (Mob skeleton : skeletons) {
				skeleton.setTarget(livingAttacker);
			}
		}
	}
}
