...
public class TraitorIdolRightclickedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (JaamsArcheologyCommonConfiguration.IDOLS.get() == true) {
			...
			if (JaamsArcheologyCommonConfiguration.TRAITORVIGOR.get() == true && !world.isClientSide()) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(JaamsArcheologyModMobEffects.TRAITOR_VIGOR.get(), 800, 0));
			}
			if (!world.isClientSide()) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
